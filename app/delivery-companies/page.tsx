...
const Page = () => {
  ...
  return (
    <DashboardShell title="شركات التوصيل" subtitle="البداية / إعدادات النظام / شركات التوصيل" hideHeaderFilters>
      <section className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-(--dash-border) bg-(--dash-panel) p-4">
          <h2 className="text-sm font-semibold text-(--dash-text)">شركات التوصيل</h2>
          <button
            type="button"
            onClick={() => {
              setEditingName(null);
        ...
        {showForm ? (
          <div className="rounded-2xl border border-(--dash-border) bg-(--dash-panel) p-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-(--dash-text)">{isEditing ? "تعديل شركة" : "إضافة شركة"}</h3>
              <button
                type="button"
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {isEditing ? "حفظ التعديل" : "إضافة شركة"}
              </button>
            </div>
          </div>
        ) : null}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-(--dash-primary) text-white">
                <tr>
                  <th className="px-4 py-3 text-right font-semibold">اسم</th>
                  <th className="px-5 py-3 text-left font-semibold">الإجراءات</th>
                </tr>
              </thead>
              <tbody>
