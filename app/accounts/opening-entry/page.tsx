...
const Page = () => {
  ...
  return (
    <DashboardShell title="القيد الافتتاحي" hideHeaderFilters>
      <section className="rounded-[32px] border border-(--dash-border) bg-(--dash-panel) px-6 py-8 shadow-[0_20px_30px_rgba(37,99,235,0.08)]">
        <div className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h1 className="text-2xl font-black text-(--dash-text)">القيد الافتتاحي</h1>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-[minmax(220px,1fr)_1fr_1fr]">
            <label className="flex flex-col gap-2 text-sm font-semibold text-(--dash-text)">
              ...
              <input
                ...
                readOnly
                value={currentDate}
                className="dash-input !rounded-full !border-(--dash-border) !bg-(--dash-panel-soft) !py-2 !px-4 !text-sm"
              />
            </label>
            <label className="flex flex-col gap-2 text-sm font-semibold text-(--dash-text)">
              <span>الفرع *</span>
              <select
                ...
                className="dash-select !rounded-full !border-(--dash-border) !py-2 !px-4 !text-sm"
              >
            ...
            <label className="flex flex-col gap-2 text-sm font-semibold text-(--dash-text)">
              ...
              <input
                ...
                className="dash-input !rounded-full !border-(--dash-border) !py-2 !px-4 !text-sm"
              />
            </label>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[200px_minmax(200px,1fr)_1fr]">
            <div className="flex flex-col gap-2 text-sm font-semibold text-(--dash-text)">
              <span>المرفقات</span>
              <button className="flex items-center justify-center gap-2 rounded-[25px] bg-(--dash-primary) px-4 py-2 text-xs font-semibold text-white shadow-(--dash-primary-soft)">
            ...
            <label className="flex flex-col gap-2 text-sm font-semibold text-(--dash-text)">
              ...
              <textarea
                ...
                className="dash-input !rounded-[20px] !border-(--dash-border) !p-4 !text-sm"
              />
            </label>
          </div>

          <div className="mt-8 space-y-4">
            <div className="flex justify-end px-1 text-sm font-semibold text-(--dash-text)">
              <span className="text-right w-full">الشجرة المحاسبية</span>
            </div>
            <div className="overflow-hidden rounded-[24px] border border-(--dash-border) bg-white shadow-sm">
              <div className="overflow-x-auto" dir="rtl">
                <table className="min-w-full text-sm">
                  ...
                  <tbody className="divide-y divide-(--dash-border) bg-[whitesmoke] text-(--dash-text)">
                    {rows.map((row) => (
                      <tr key={row.id} className="bg-white">
                        ...
                        <td className="px-3 py-3 text-center">
                          <select
                            ...
                            className="w-full rounded-full border border-(--dash-border) bg-(--dash-panel) px-3 py-1 text-xs text-right text-(--dash-text)"
                          >
                        ...
                        <td className="px-3 py-3 text-center">
                          <input
                            ...
                            className="w-full rounded-full border border-(--dash-border) bg-[#fefbf4] px-3 py-1 text-xs text-right text-(--dash-text)"
                          />
                        </td>
                        <td className="px-3 py-3 text-center">
                          <input
                            ...
                            className="w-full rounded-full border border-(--dash-border) bg-[#f8f9f8] px-3 py-1 text-xs text-right text-(--dash-text)"
                          />
                        </td>
                      </tr>
                    ))}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <button
                type="button"
                onClick={addRow}
                className="rounded-[25px] bg-(--dash-primary) px-4 py-2 text-sm font-semibold text-white shadow-(--dash-primary-soft)"
              >
                إضافة حساب
              </button>
              <button
                type="button"
                className="rounded-[25px] border border-[#0a4225] bg-[white] px-5 py-2 text-sm font-semibold text-[#0a4225]"
              >
                إتمام العملية
              </button>
            </div>
